...
def cards3(title,cards,kicker):
    ...
    if len(cards)<=3:
        ...
        positions=[(Inches(0.85),Inches(1.85)),(Inches(6.75),Inches(1.85)),
                   (Inches(0.85),Inches(4.25)),(Inches(6.75),Inches(4.25))]
        for i,c in enumerate(cards[:4]):
            x,y=positions[i]; col=c[0]
            rect(s,x,y,Inches(5.75),Inches(2.1),_panel()); rect(s,x,y,Inches(0.1),Inches(2.1),col)
            txt(s,x+Inches(0.24),y+Inches(0.18),Inches(5.2),Inches(0.42),[[(c[1],17,col,True)]])
            bullets(s,x+Inches(0.24),y+Inches(0.68),Inches(5.2),Inches(1.2),c[2],size=12,color=_ink(),mcolor=col,gap=5)
    footer(s); return s
...
def activity_overview(tag,title,desc,build,services,kicker):
    ...
    # Long connected-lab filenames and manifests need a real multi-line box.
    # Keep resource detail in the matching lab instead of letting it collide
    # with the deliverable text on the slide.
    bL=len(str(build)); bsize = 17 if bL<=190 else (15 if bL<=300 else 13)
    txt(s,Inches(1.1),Inches(4.86),Inches(11),Inches(0.92),[[(build,bsize,_ink(),True)]])
    txt(s,Inches(1.1),Inches(5.84),Inches(11.2),Inches(0.34),[[("Resources:  ",14,_grey(),True),("Use the matching lab's prerequisites and supplied files.",14,_grey(),False)]]); footer(s); return s
...
_dl=flow_h("Download Course Material",[
 "Open the C1382 course page",
 "Follow the courseware links",
 "Download trainer and learner slides",
 "Download the Learner Guide and Lesson Plan",
 "Open Activities alongside the guide"],kicker="COURSE MATERIALS")
# Full course URL as a wide caption (kept out of the narrow chips so it never wraps mid-token)
txt(_dl,Inches(0.85),Inches(6.5),Inches(11.6),Inches(0.4),
    [[("Course page:  "+getattr(C,"COURSE_URL","https://www.tertiarycourses.com.sg/"),14,_acc(BLUE),True)]],align=PP_ALIGN.CENTER)
...
_right=[(f"Day {_d2} — {C.DAY_THEMES[_d2]}",0)]+[_topic_line(_TB[n]) for n in _BY_DAY.get(_d2,[]) if n in _TB]+[
 ("Daily timing",0),(getattr(C,"DAILY_TIMING","8 scheduled hours per day, excluding lunch"),1)]
_hours=getattr(C,"DAY_MINUTES",480)/60
if C.DAYS == 1:
    content(f"Lesson Plan — 1 Day, {_hours:g} hours",[
        f"Day 1 — {C.DAY_THEMES[1]}",
        *[line for line,_indent in _left[1:]],
        "Daily timing — "+getattr(C,"DAILY_TIMING",f"{_hours:g} scheduled hours, excluding lunch"),
    ],kicker="SCHEDULE",size=17)
else:
    two_col(f"Lesson Plan — {C.DAYS} Days, {_hours:g} hours/day",_left,_right,
...
for t in C.TOPICS:
    ...
    # Course-specific knowledge sections are authored once in course_data.py
    # and rendered as visual frameworks here and as detailed study text in the
    # Learner Guide. This keeps the deck concept-first without duplicating text.
    for ks in t.get("teaching", []):
        kind=ks.get("kind", "tiles")
        kicker=ks.get("kicker", f"TOPIC {t['code']} · CONCEPT")
        if kind == "flow":
            flow_h(ks["title"], ks["visual"], kicker=kicker, color=accent)
        elif kind == "compare":
            two_col(ks["title"], ks["left"], ks["right"], kicker=kicker,
                    lhead=ks.get("left_title", "Option A"),
                    rhead=ks.get("right_title", "Option B"))
        else:
            tile_grid(ks["title"], ks["visual"], kicker=kicker,
                      cols=2, size=14, accent=accent)
    ...
    for a in acts:
        ...
        _lab_extras(a["num"])
        # Keep click-by-click detail in labs/*.md and the Learner Guide. The
        # deck shows a single, readable route plus the evidence required to
        # verify the result, preserving a knowledge-to-practice ratio above 60%.
        route=list(a.get("deck_steps", [si for si,_sc in a["steps"][:4]]))
        route.append(a.get("deck_verify", "Run the Test It checks and save the verified checkpoint."))
        flow_h(f"Lab {a['num']} — Key Route", route,
               kicker=f"TOPIC {t['code']} · PRACTICE ROUTE", color=accent)
    # topic recap
...
content("Keep Practising After the Course",[
 ...
 "Open the current C1382 materials from the course page or repository.",
...
# python-pptx writes the correct 16:9 dimensions but can retain a stale 4:3
# semantic label and zero slide count in the package metadata. Keep both the
# presentation XML and extended properties consistent with the actual deck.
prs._element.sldSz.set("type", "screen16x9")

OUT=os.path.join(REPO,"courseware",f"{C.SHORT_TITLE}-{C.VERSION}.pptx")
prs.save(OUT)

with zipfile.ZipFile(OUT, "r") as _source:
    _members = [(_item, _source.read(_item.filename)) for _item in _source.infolist()]
_fd, _temp = tempfile.mkstemp(suffix=".pptx", dir=os.path.dirname(os.path.abspath(OUT)))
os.close(_fd)
try:
    with zipfile.ZipFile(_temp, "w") as _target:
        for _item, _data in _members:
            if _item.filename == "docProps/app.xml":
                _xml = _data.decode("utf-8")
                _xml = re.sub(r"(<Slides>).*?(</Slides>)", fr"\g<1>{len(prs.slides)}\g<2>", _xml, count=1)
                _xml = re.sub(
                    r"(<PresentationFormat>).*?(</PresentationFormat>)",
                    r"\g<1>On-screen Show (16:9)\g<2>",
                    _xml,
                    count=1,
                )
                _data = _xml.encode("utf-8")
            _target.writestr(_item, _data)
    os.replace(_temp, OUT)
finally:
